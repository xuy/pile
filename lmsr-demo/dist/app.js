 ...
 */
import { calculatePrice, calculateTradeCost, calculatePriceImpact, calculateMaxLoss, calculateAllPrices } from './core/lmsr.js';
import { createMarketState, executeMarketTrade, calculatePnL, resetMarket, calculateExpectedValue } from './core/market.js';
...
function initializeCharts() {
    ...
    probabilityChart = new window.Chart(probabilityCtx, {
        type: 'bar',
        data: {
            labels: [],
            datasets: [{
...
function updateCharts() {
    ...
    probabilityChart.data.datasets[0].backgroundColor = currentState.market.outcomes.map((_, i) => colors[i % colors.length].replace('rgb', 'rgba').replace(')', ', 0.7)'));
    probabilityChart.data.datasets[0].borderColor = currentState.market.outcomes.map((_, i) => colors[i % colors.length]);
