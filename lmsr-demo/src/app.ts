...
import { scenarios, getScenario, Scenario } from './scenarios/scenarios.js';
...
function setupEventListeners() {
  ...
  const outcomeSelect = document.getElementById('outcome-select') as HTMLSelectElement;
  const sharesInput = document.getElementById('shares-input') as HTMLInputElement;
  const executeTrade = document.getElementById('execute-trade') as HTMLButtonElement;
  const resetButton = document.getElementById('reset-market') as HTMLButtonElement;
  const liquiditySlider = document.getElementById('liquidity-slider') as HTMLInputElement;
...
function loadScenario(scenario: Scenario) {
  ...
  const slider = document.getElementById('liquidity-slider') as HTMLInputElement;
  slider.value = scenario.market.b.toString();
  updateLiquidityDisplay(scenario.market.b);
}

// Update scenario info display
function updateScenarioInfo(scenario: Scenario) {
  const info = document.getElementById('scenario-info') as HTMLDivElement;
  const question = document.getElementById('market-question') as HTMLHeadingElement;
...
function updateMarketDisplay() {
  const container = document.getElementById('market-outcomes') as HTMLDivElement;
...
function updatePortfolio() {
  const cashEl = document.getElementById('cash-value') as HTMLSpanElement;
  const holdingsList = document.getElementById('holdings-list') as HTMLDivElement;
  const portfolioValue = document.getElementById('portfolio-value') as HTMLSpanElement;
  const pnlEl = document.getElementById('pnl-value') as HTMLSpanElement;
...
function handleLiquidityChange(event: Event) {
...
function updateLiquidityDisplay(b: number) {
  const valueEl = document.getElementById('liquidity-value') as HTMLSpanElement;
  const hintEl = document.getElementById('liquidity-hint') as HTMLSpanElement;
...
function addTradeToHistory(trade: any) {
  const container = document.getElementById('trade-history') as HTMLDivElement;
...
// Update guidance
function updateGuidance(scenario: Scenario) {
  const container = document.getElementById('guidance-content') as HTMLDivElement;
...
function initializeCharts() {
  const priceCtx = (document.getElementById('price-chart') as HTMLCanvasElement).getContext('2d')!;
  const probabilityCtx = (document.getElementById('probability-chart') as HTMLCanvasElement).getContext('2d')!;

  priceChart = new (window as any).Chart(priceCtx, {
    type: 'line',
    data: {
      labels: [],
      datasets: []
  ...
  probabilityChart = new (window as any).Chart(probabilityCtx, {
    type: 'bar',
    data: {
      labels: [],
      datasets: [{
